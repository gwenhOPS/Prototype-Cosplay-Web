  ...
  useEffect(() => {
    const fetchKostum = async () => {
      try {
        ...
        setKostumRekomendasi(data.slice(0, 10));
      } catch (err) {
            ...
            >
              {kostumRekomendasi.map((item) => (
                <SwiperSlide key={item.id}>
                  <div
                    className={`rounded-xl shadow-lg overflow-hidden border p-2 transition-colors ${
                      isDark
                  ...
                  >
                    <img
                      ...
                      alt={item.ProductName || item.nama || "Kostum"}
                      className="h-48 w-full object-cover rounded-md"
                    />
                    <div className="p-3">
                      <h3 className="font-semibold text-lg mb-1 truncate">
                        {item.ProductName || item.nama || "Tanpa Nama"}
                      </h3>
                      <p className="text-sm text-gray-400 mb-2">
                        Rp {(item.Harga || item.harga || 0).toLocaleString()}
                      </p>
                      <div className="flex items-center gap-1 text-yellow-400">
                        <Star size={16} />{" "}
                        <span>{(item.Rating || 4.8).toFixed(1)}</span>
                      </div>
                      <button className="mt-3 w-full py-1.5 rounded-md font-medium bg-pink-600 hover:bg-pink-700 text-white transition">
                        Sewa Sekarang
                      </button>
                    </div>
                  </div>
                ...
                >
                  ...
                  <div>
                    <p className="font-medium">
                      {item.namaProduk || item.nama || "Kostum Tidak Diketahui"}
                    </p>
...
function StatBox({ icon, label, value, isDark }) {
  return (
    <motion.div
      whileHover={{ y: -5 }}
      className={`flex flex-col items-center justify-center py-6 rounded-xl shadow-lg border transition-colors ${
        isDark
          ? "bg-gray-800 border-gray-700 text-gray-100"
          : "bg-white border-gray-200 text-gray-900"
      }`}
